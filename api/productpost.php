    ...
    if($jwt!=NULL && $decoded->iss=="https://flipin-store-api.herokuapp.com/" && $decoded->exp > $now->getTimestamp()){
        ...
        if($cat=="C" || $cat=='C'){
            ...
            else if(trim($url)=="")
            {
                ...
                $insert = "Insert into productpost(productname, category, description, customerid, status, mediaurl, postviews) values(
                    '$name','$category','$desc',$cid,'OPEN','$url',0);";
                $datainsert = pg_query($pdo, $insert);
